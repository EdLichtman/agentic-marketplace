...
**D. Rank / bubble (EDHREC-style).** Once a need's candidate pool exists, rank it the way we lean on EDHREC today. Deterministic signal already on every Scryfall card: **`edhrec_rank`** (lower = more played) and `order=edhrec` in search. Bubble the strongest to the top; keep the jank visible below the fold.

**EDHREC data source (pulled from the sibling repo so RankNode can be built from this spec alone, no need to re-cross-reference):**
- **JSON API** (no JS rendering needed, unlike `edhrec.com` pages directly): `https://json.edhrec.com/pages/commanders/<slug>.json`. Slug derivation: lowercase the commander name, replace spaces with hyphens, strip commas/apostrophes (e.g. `"Atraxa, Praetors' Voice"` → `atraxa-praetors-voice`; partner commanders → `<slug1>/<slug2>`).
- **Inclusion % vs "lift" (formerly "synergy %")** — EDHREC reports both per card. *Inclusion %* = generically popular (good but not commander-specific). The commander-specific signal — *synergy %* — was **replaced by "lift" around Dec 2025** ([EDHREC: "From Synergy to Lift"](https://edhrec.com/articles/from-synergy-to-lift-the-math-behind-edhrecs-new-era)): lift measures how often a card co-occurs with the commander vs. random chance, range 0→∞, median ~1.58, **logarithmic and symmetric** (lift 2.0 = appears 2x as often as random; lift 0.5 = appears half as often — equally extreme in opposite directions; lift 1.0 = independent/random). This fixes the old synergy score's bug where ubiquitous staples (Sol Ring, Command Tower) showed false strong-negative synergy with everything. RankNode should prefer lift (>1, the higher the better) when bubbling for a specific commander, falling back to `edhrec_rank`/inclusion for general staples. **Unverified:** whether `json.edhrec.com`'s field is literally named `lift` or still `synergy` — confirm by fetching a live `<slug>.json` when RankNode is built.
- **Average-deck page**: `https://edhrec.com/average-decks/<slug>` (or its JSON equivalent) gives a ready-made meta baseline decklist — useful as a NeedsNode/PoolNode seed or a sanity-check baseline.
- **Fallback** if the JSON endpoint 404s or a commander page doesn't exist: WebSearch `"<commander name>" EDHREC top cards high synergy`.
- (Source: `magic-cards-edh-deck-main/.claude/commands/scrape-meta.md` and `fetch-decklist.md`.)
